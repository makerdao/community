...
**È importante capire che 150% è il rapporto minimo, tra il valore del proprio collateral e quello dei Dai creati, scesi sotto il quale il CDP verrà messo in stato di liquidazione dai Keeper.**
...
Essa è una penalitàche viene aggiunta al debito totale al momento della liquidazione e che viene prelevata dal collateral del CDP.

I proventi delle penalità di liquidazione sono trasferiti alla riserva di token PETH (PETH pool). Questo aumenta il rapporto con i token WETH che gli utenti ricevono quando rimuovono il collateral dal proprio CDP. Quindi la penalità aumenta il valore della riserva di PETH del sistema in periodi durante i quali ci sono molte liquidazioni a causa, ad esempio, dell’instabilità dei mercati.


# Cosa succede durante una liquidazione?
...
L’ordine delle operazioni eseguite è il seguente:
...
* Il PETH rimosso viene messo in vendita su dai.makerdao.com ad un prezzo scontato, per facilitarne la vendita. Lo sconto è chiamato Boom/Bust Spread.
* I Dai ricavati dalla vendita del PETH sono distrutti per appianare il debito del CDP.
* Se la vendita ha prodotto un eccesso di Dai, questi vengono venduti per ottenere PETH, i quali sono successivamente distrutto per aumentare il valore dei PETH in esistenza.
* Se la vendita non ha prodotto una quantità sufficiente di Dai, allora dei PETH della riserva vengono venduti per coprire la differenza. Questo riduce il valore complessivo dei PETH in esistenza.
...
* Il prezzo di ETH comunicato dagli Oracoli è di 350USD
...
Quale delle due opzioni sia la migliore dipende dai tuoi obiettivi nel lungo termine. Se sei convinto che in futuro il valore del tuo collateral aumenterà, puoi aggiungere del collateral al CDP per riportarlo in stato di sicurezza. Se invece vuoi ridurre la tua esposizione al rischio di volatilità del prezzo del collateral, puoi semplicemente ripagare parzialmente il tuo debito rimettendo Dai sul tuo CDP.
...
Evidentemente il prezzo di liquidazione viene ridotto molto di più ripagando il debito di Dai piuttosto che aggiungendo del collateral.
...
* Attiva degli avvisi nelle tue app preferite, o in varie app, in modo tale da essere sempre al corrente degli eventi del mercato.
...
* Se credi che il mercato possa entrare in un periodo di discesa, puoi ritirare collateral in eccesso e venderlo in cambio di Dai e ripagare il tuo debito. *Fai attenzione a mantenere sempre una distanza di sicurezza dal tuo prezzo di liquidazione, giacché questa procedura aumenterà temporaneamente il rischio del tuo CDP fintanto che non ripagherai il tuo debito.*

Ricordati che aprire un CDP comporta necessariamente un rischio. La quantità di rischio che tu puoi accettare di assumere dipende da molti fattori. La determinazione di un corretto [risk profile](https://www.investopedia.com/terms/r/risk-profile.asp) è effettivamente una scienza, ma a volte i possessori di CDP devono fare le proprie valutazioni in autonomia.

Per ulteriori informazioni sul rischio, consulta le condizioni di utilizzo [Terms of Service](https://cdp.makerdao.com/terms) le quali contengono importanti informazioni di natura legale. Ogni possessore di CDP deve accettare le condizioni di utilizzo prima di usare il sistema Dai.


# Come funziona la vendita del collateral effettuata dallo smart contract?

Quando un Keeper liquida un CDP insicuro, il Liquidity Providing Contract (LPC) si assicura di mettere in vendita il collateral sul Dai Dashboard. Il prezzo di vendita è determinato dal prezzo del collateral comunicato dagli Oracoli al quale viene applicato un modificatore. Questo modificatore è normalmente uno sconto, il quale viene applicato al debito complessivo che deve essere recuperato. Questa sconto determina una differenza (‘spread’) rispetto al prezzo di mercato del collateral, e serve a facilitare una vendita veloce e una pronta ricapitalizzazione del sistema.

Gli utenti possono acquistare i PETH che sono stati in precedenza inseriti dal Liquidity Providing Contract (LPC) nel Dai Dashboard. Ogni surplus di Dai ottenuto dalla vendita può essere acquistato con PETH.


# Posso acquistare del PETH precedentemente prelevato?

Sul Dai dashboard c’è una sezione chiamata “Total Liquidity Available from forced CDP liquidations” (Liquidità totale risultante da liquidazioni forzate di CDP) nella quale i partecipanti possono acquistare PETH ottenuti da liquidazioni ad un prezzo scontato determinato dal “Bust/Boom Spread”.
...
* Monitoraggio delle informazioni degli Oracoli: https://mkr.tools/system/feeds
